...
import frLunrLanguage from 'lunr-languages/lunr.fr'
import esLunrLanguage from 'lunr-languages/lunr.es'

!lunr.stemmerSupport && lunrStemmer(lunr)
!lunr.fr && frLunrLanguage(lunr)
!lunr.es && esLunrLanguage(lunr)
...
export default {
  props: {
    ...
    lang: {
      type: String,
      default: '',
      validator: val => ['', 'en', 'fr', 'es'].includes(val)
    },
    locale: {
      type: String,
      default: '',
      validator: val => ['', 'en', 'fr', 'es'].includes(normalizeLanguage(val))
    }
  },
  data () {
    return {

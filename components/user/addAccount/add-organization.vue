        ...
        <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition" persistent>
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="rightAddBtn">
                    <v-icon start icon="fas fa-plus"></v-icon>Add Organization
                </v-btn>
            </template>
            <v-card>
                <form method="post" @v-on:submit.prevent="addOrganization()">
                    <v-toolbar dark color="primary">
                        <v-btn icon dark @click="dialog = false">
                            <v-icon icon="fas fa-circle-xmark"></v-icon>
                        </v-btn>
                        <v-card-title>
                            <span class="text-h6">Create new Organization</span>
                        </v-card-title>
                    </v-toolbar>
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="6">
                                    <v-text-field v-model="name" id="categoryName" label="Organization Name*" required></v-text-field>
                                </v-col>
                                <v-col cols="6">
                                ...
                                <v-col cols="12" sm="6">
                                    <v-file-input accept="image/*" label="Organization Image"></v-file-input>
                                </v-col>
                            </v-row>
                        </v-container>
            ...
            addOrganization() {
                ...
                this.$apollo.mutate({
                    ...
                    update: (store, {
                        ...
                        data.allCategories.push(addOrganization)
                        store.writeQuery({
                            query: findManyCategories,
                            variables: {
                                first: 5,

        ...
        <v-dialog v-model="dialog">
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="rightAddBtn">
                    <v-icon start icon="fas fa-plus"></v-icon>Generate new token
                </v-btn>
            </template>
            <v-card>
                <form method="post" @v-on:submit.prevent="addAccessToken()">
                    <v-toolbar dark color="primary">
                        ...
                        <v-card-title>
                            <span class="text-h6">Generate New Token</span>
                        </v-card-title>
                    </v-toolbar>
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="6">
                                    <v-text-field v-model="name" id="tokenName" label="Name*" hint="Please enter a name for this token" required></v-text-field>
                                </v-col>
                                <v-col cols="6">
                                    <v-text-field v-model="content" label="Generated Token" id="tokenDescription" disabled>
                                    </v-text-field>
                                </v-col>
                            </v-row>
                        </v-container>

<template>
    <v-row justify="center">
        <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition" persistent>
            ...
            <v-card>
                <form method="post" @v-on:submit.prevent="addProject()">
                    ...
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="6">
                                    <v-text-field v-model="name" id="categoryName" label="Project Name*" required></v-text-field>
                                </v-col>
                                <v-col cols="6">
                                ...
                                <v-col cols="12" sm="6">
                                    <v-file-input accept="image/*" label="Project Image"></v-file-input>
                                </v-col>
                            </v-row>
                        </v-container>
            ...
            addProject() {
                ...
                this.$apollo.mutate({
                    ...
                    update: (store, {
                        ...
                        data.allCategories.push(addProject)
                        store.writeQuery({
                            query: findManyCategories,
                            variables: {
                                first: 5,

        ...
        <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition" persistent>
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="rightAddBtn">
                    <v-icon start icon="fas fa-plus"></v-icon>Add Account
                </v-btn>
            </template>
            <v-card>
                <form method="post" @v-on:submit.prevent="addAccount()">
                    ...
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="6">
                                    <v-text-field v-model="name" id="categoryName" label="Account Name*" required></v-text-field>
                                </v-col>
                                <v-col cols="6">
                                ...
                                <v-col cols="12" sm="6">
                                    <v-file-input accept="image/*" label="Account Image"></v-file-input>
                                </v-col>
                            </v-row>
                        </v-container>
            ...
            addAccount() {
                ...
                this.$apollo.mutate({
                    ...
                    update: (store, {
                        ...
                        data.allCategories.push(addAccount)
                        store.writeQuery({
                            query: findManyCategories,
                            variables: {
                                first: 5,

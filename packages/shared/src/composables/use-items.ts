...
export function useItems(collection: Ref<string | null>, query: ComputedQuery): UsableItems {
	...
	async function getItems() {
		...
		try {
			 ...
			 * able to render out the alternatecms_files collection (file library) using regular layouts
			 *
			 * Layouts expect the file to be a m2o of a `file` type, however, alternatecms_files is the
			 ...
			 */
			if (collection.value === 'alternatecms_files') {

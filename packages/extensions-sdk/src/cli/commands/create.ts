...
} from '@alternatecms/shared/constants';
...
import {
	...
	ExtensionType,
	HybridExtensionType,
} from '@alternatecms/shared/types';
...
async function createPackageExtension({
	...
	const spinner = ora(chalk.bold('Scaffolding Alternatecms extension...')).start();
...
function getPackageManifest(name: string, options: ExtensionOptions, deps: Record<string, string>) {
	const packageManifest: Record<string, any> = {
		name: EXTENSION_NAME_REGEX.test(name) ? name : `alternatecms-extension-${name}`,
		description: 'Please enter a description for your extension',
		icon: 'extension',
		version: '1.0.0',
		keywords: ['alternatecms', 'alternatecms-extension', `alternatecms-custom-${options.type}`],
		[EXTENSION_PKG_KEY]: options,
		scripts: {
			build: 'alternatecms-extension build',
			dev: 'alternatecms-extension build -w --no-minify',
			link: 'alternatecms-extension link',
		},
		devDependencies: deps,
	};

	if (options.type === 'bundle') {
		packageManifest.scripts['add'] = 'alternatecms-extension add';
	}

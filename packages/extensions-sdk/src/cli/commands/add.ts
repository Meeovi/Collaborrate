...
import { isIn, isTypeIn } from '@alternatecms/shared/utils';
import { pathToRelativeUrl } from '@alternatecms/shared/utils/node';
import {
	EXTENSION_LANGUAGES,
...
export default async function add(): Promise<void> {
	...
	if (extensionOptions.type === 'bundle') {
		...
		const spinner = ora(chalk.bold('Modifying Alternatecms extension...')).start();
		...
		const newExtensionManifest = {
			...extensionManifest,
			name: EXTENSION_NAME_REGEX.test(extensionName) ? extensionName : `alternatecms-extension-${extensionName}`,
			keywords: ['alternatecms', 'alternatecms-extension', `alternatecms-custom-bundle`],
			[EXTENSION_PKG_KEY]: newExtensionOptions,
			devDependencies: await getExtensionDevDeps(

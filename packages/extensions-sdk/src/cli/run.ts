...
program.name('alternatecms-extension').usage('[command] [options]');
program.version(pkg.version, '-v, --version');

program
	.command('create')
	.arguments('<type> <name>')
	.description('Scaffold a new Alternatecms extension')
	.option('-l, --language <language>', 'specify the language to use')
	.action(create);

program.command('add').description('Add entries to an existing Alternatecms extension').action(add);

program
	.command('build')
	.description('Bundle a Alternatecms extension to a single entrypoint')
...
program
	.command('link')
	.description('Creates a symlink to the extension in the Alternatecms extensions folder')
	.argument('<path>', 'path to the extension folder of alternatecms')
	.action(link);
